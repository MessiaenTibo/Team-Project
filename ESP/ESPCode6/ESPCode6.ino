...
void connect_mqttServer() {
  ...
  while (!client.connected()) {
        ...
        // Attempt to connect
        if (client.connect("ESP32_client6")) { // Change the name of client here if multiple ESP32 are connected
          //attempt successful
          Serial.println("connected");
          // Subscribe to topics here
          client.subscribe("rpi/broadcast");
          client.subscribe("esp32/kleur6");
...
void loop() { 
  ...
  currentState = digitalRead(BUTTON_PIN);
    ...
    {
      client.publish("esp32/sensor6", "gedrukt");
    }
  // save the last state

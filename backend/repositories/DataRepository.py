...
class DataRepository:
    ...
    def read_1vs1_data_by_time(time):
        sql = "SELECT idSpel, spelNaam, tijd, winnaar, count(winnaar) as 'score' FROM spel where spelNaam = '1VS1' and tijd = %s group by winnaar order by winnaar desc"
        params = [time]
    ...
    def read_speedrun_data_by_difficulty_and_buttons(difficulty, buttons):
        sql = "SELECT idSpel, spelNaam, naam1, winnaar, TIME(tijd) AS 'score', aantalPalen, moeilijkheidsgraad FROM spel where spelNaam = 'Speedrun' and moeilijkheidsgraad = '%s' and aantalPalen = '%s'  order by TIME(tijd) asc"
